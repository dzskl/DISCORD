...
router.put('/:id', requireAuth, async (req, res) => {
  ...
  require('../services/audit.service').log({ req, action: 'product.update', target_type: 'product', target_id: req.params.id, details: req.body });
  ...
  if (wasOutOfStock && willHaveStock) {
    notifyWishlist(parseInt(req.params.id)).catch(e => require('../utils/logger').warn({ err: e }, 'falha ao notificar wishlist'));
    announceRestock(parseInt(req.params.id), newStock).catch(e => require('../utils/logger').warn({ err: e }, 'falha ao anunciar restock'));
  }
